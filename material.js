...
const MATERIAL_XLSX_COLUMNS = [
  ...
  { key: "amount", title: "金额" },
  { key: "isReady", title: "是否齐备" },
  { key: "supplier", title: "供应商" },
  { key: "status", title: "状态" },
  { key: "promiseDate", title: "承诺交期" },
  { key: "actualDate", title: "实际到货" },
  { key: "abnormalReason", title: "异常原因" },
  { key: "abnormalAltMaterial", title: "替代料" },
  { key: "abnormalRecoverDate", title: "预计恢复时间" },
...
if (typeof MES_SHARED.applyStaticSeed === "function") {
  MES_SHARED.applyStaticSeed({
    allowKeys: [STORAGE_KEY, ORDER_STORAGE_KEY, EXTRA_KEY, SUPPLIER_CUSTOM_KEY, MATERIAL_CUSTOM_KEY],
    preferExisting: true,
  });
}
...
function bindHeaderActions() {
  ...
  const exportBtn = document.getElementById("materialExportBtn");
  const importInput = document.getElementById("materialImportInput");
  ...
  if (exportBtn) exportBtn.addEventListener("click", exportMaterialXlsx);
  if (importInput) importInput.addEventListener("change", importMaterialXlsx);
}
...
function normalizeImportNumber(value, { integer = false } = {}) {
  if (value == null || value === "") return "";
  const n = Number(value);
  if (!Number.isFinite(n)) return "";
  if (integer) return Math.max(0, Math.floor(n));
  return Number(Math.max(0, n).toFixed(2));
}

function normalizeImportStatus(value, row, extra) {
...
function getMaterialImportMatchKey(item = {}) {
  ...
  const spec = String(item.spec || "").trim().toUpperCase();
  if (!orderNo && !material && !spec) return "";
  return `biz:${orderNo}|${material}|${spec}`;
}

function sanitizeImportedGroups(rawGroups, row, extra) {
    ...
    .map((group) => {
      const lines = Array.isArray(group?.lines)
        ? group.lines
          .map((line) => ({
            size: String(line?.size || "").trim(),
            qty: normalizeImportNumber(line?.qty, { integer: true }),
            amount: normalizeImportNumber(line?.amount, { integer: false }),
          }))
          .filter((line) => line.size || line.qty !== "" || line.amount !== "")
        : [];
      return {
        itemKind: String(group?.itemKind || "material"),
        material: String(group?.material || "").trim(),
        supplier: String(group?.supplier || "").trim(),
        supplierLink: String(group?.supplierLink || "").trim(),
        amount: normalizeImportNumber(group?.amount, { integer: false }),
        status: normalizeImportStatus(group?.status, row, extra),
        orderedAt: normalizeImportDateText(group?.orderedAt),
        purchasedAt: normalizeImportDateText(group?.purchasedAt),
        arrivedAt: normalizeImportDateText(group?.arrivedAt),
        screenshot: String(group?.screenshot || "").trim(),
        lines,
      };
    })
    .filter((group) => group.material || group.supplier || group.lines.length > 0 || group.amount !== "");
}

function exportMaterialXlsx() {
  if (!window.XLSX) {
    showInfo("缺少 xlsx.full.min.js，无法导出。", "导出失败");
    return;
  }
  const rowsToExport = rows.map((row) => {
    ...
    return {
      ...
      status: normalizeStatus(extra.status, row, extra),
      promiseDate: extra.promiseDate || "",
      actualDate: extra.actualDate || "",
      abnormalReason: extra.abnormalReason || "",
      abnormalAltMaterial: extra.abnormalAltMaterial || "",
      abnormalRecoverDate: extra.abnormalRecoverDate || "",
  ...
  const tableRows = rowsToExport.map((item) => {
    const result = {};
    MATERIAL_XLSX_COLUMNS.forEach(({ key, title }) => {
      result[title] = item[key] ?? "";
    });
    return result;
  });
  const ws = XLSX.utils.json_to_sheet(tableRows, { header: MATERIAL_XLSX_COLUMNS.map((col) => col.title) });
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, "采购明细");
  XLSX.writeFile(wb, `mes_materials_${new Date().toISOString().slice(0, 19).replace(/[T:]/g, "-")}.xlsx`);
}

async function importMaterialXlsx(event) {
  const input = event?.target;
  const file = input?.files?.[0];
  if (!file) return;
  if (input) input.value = "";
  if (!window.XLSX) {
    showInfo("缺少 xlsx.full.min.js，无法导入。", "导入失败");
    return;
  }
  try {
    ...
    if (!Array.isArray(sheetRows) || sheetRows.length === 0) {
      showInfo("导入文件为空。", "导入失败");
      return;
    }

    const titleToKey = Object.fromEntries(MATERIAL_XLSX_COLUMNS.map((col) => [col.title, col.key]));
    const importedItems = sheetRows
      .map((sheetRow) => {
        const mapped = {};
        Object.entries(sheetRow || {}).forEach(([title, value]) => {
          const key = titleToKey[String(title).trim()];
      ...
      .filter((item) =>
        ...
        || String(item.groupsJson || "").trim()
      );

    if (importedItems.length === 0) {
      showInfo("未识别到可导入数据，请使用系统导出的模板。", "导入失败");
      return;
    }
    ...
    mergedRows.forEach((row, index) => {
      const rowExtra = mergedExtras[row.id] || getExtra(row.id);
      const keyById = getMaterialImportMatchKey({ id: row.id });
      if (keyById) indexByKey.set(keyById, index);
      const keyByBiz = getMaterialImportMatchKey({ orderNo: row.orderNo, material: row.material, spec: row.spec });
      if (keyByBiz && !indexByKey.has(keyByBiz)) indexByKey.set(keyByBiz, index);
      mergedExtras[row.id] = rowExtra;
    });
    ...
    importedItems.forEach((item) => {
      const incomingOrderNoRaw = String(item.orderNo || "").trim().toUpperCase();
      const normalizedOrderNo = incomingOrderNoRaw ? (normalizeOrderNo(incomingOrderNoRaw) || incomingOrderNoRaw) : "";
      const incomingMaterial = String(item.material || "").trim();
      const incomingSpec = String(item.spec || "").trim();
      const key = getMaterialImportMatchKey({
        id: String(item.id || "").trim(),
        orderNo: normalizedOrderNo,
        material: incomingMaterial,
        spec: incomingSpec,
      });
      const hitIndex = key ? indexByKey.get(key) : undefined;
      ...
      if (groupsJsonText) {
        try {
          ...
          groups = sanitizeImportedGroups(parsed, row, baseExtra);
        } catch {
          groups = [];
        }
      }
      ...
      const summary = summarizeGroupStatus(row, baseExtra, groups);
      const isReadyText = String(item.isReady || "").trim();
      row.isReady = isReadyText || summary.isReady;

      const nextExtra = {
        ...baseExtra,
        supplier: String(item.supplier || serialized.supplier || baseExtra.supplier || "").trim(),
        status: normalizeImportStatus(String(item.status || summary.status || baseExtra.status || "").trim(), row, baseExtra),
        promiseDate: normalizeImportDateText(item.promiseDate || baseExtra.promiseDate),
        actualDate: normalizeImportDateText(item.actualDate || baseExtra.actualDate),
        abnormalReason: String(item.abnormalReason || baseExtra.abnormalReason || "").trim(),
        abnormalAltMaterial: String(item.abnormalAltMaterial || baseExtra.abnormalAltMaterial || "").trim(),
        abnormalRecoverDate: normalizeImportDateText(item.abnormalRecoverDate || baseExtra.abnormalRecoverDate),
      };

      if (hitIndex != null) {
        mergedRows[hitIndex] = row;
        updateCount += 1;
      } else {
        mergedRows.push(row);
        insertCount += 1;
      }
      mergedExtras[row.id] = nextExtra;
      changedRows.push(row);

      const keyById = getMaterialImportMatchKey({ id: row.id });
      if (keyById) indexByKey.set(keyById, hitIndex != null ? hitIndex : (mergedRows.length - 1));
      const keyByBiz = getMaterialImportMatchKey({ orderNo: row.orderNo, material: row.material, spec: row.spec });
      if (keyByBiz) indexByKey.set(keyByBiz, hitIndex != null ? hitIndex : (mergedRows.length - 1));
    });
    ...
    cleanupExtras();
    resetDerivedCaches();
    await persist({ changed: changedRows });
    render();
    showInfo(`导入成功：新增 ${insertCount} 条，覆盖 ${updateCount} 条。`, "导入完成");
  } catch (error) {
    console.error("导入采购明细失败", error);
    showInfo("导入失败：请使用系统导出的 Excel 模板或检查文件内容。", "导入失败");
  }
}
